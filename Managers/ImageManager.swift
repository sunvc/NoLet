...
class ImageManager {
    ...
    class func downloadFile(from mediaUrl: String, proxy proxyServer: PushServerModel? = nil) async throws -> URL {
        ...
        var config: CryptoModelConfig?{
            if proxyServer.url == BaseConfig.server{ return .data }
            if let sign = proxyServer.sign{  return CryptoModelConfig(inputText: sign) }
            return nil
        }
        
        guard let config = config,
              let signStr = CryptoManager(.data).encrypt(mediaUrl)?.safeBase64 else {
            throw "invalid sign"
        }
        ...
        // 添加默认头
        request.setValue(BaseConfig.customUserAgent, forHTTPHeaderField: "User-Agent")
        ...
        if let signStr = CryptoManager(.data).encrypt(mediaUrl)?.safeBase64{
