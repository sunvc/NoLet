...
class NetworkManager: NSObject {
    ...
    func test(url: String = "https://example.com") async -> Bool {
        guard
            let (_, response) = try? await self.fetch( url: url, method: .HEAD, timeout: 3),
    ...
    func health(url: String) async -> Bool {
        guard let data = try? await self.fetch(url: url + "/health", method: .GET, timeout: 3), let response = data.1 as? HTTPURLResponse else {
            return false
        }
    ...
    func download(from fileUrl: URL, headers:[String: String] = [:], timeout:Double = 15) async throws -> URL {
