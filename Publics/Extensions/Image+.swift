...
extension UIImage {
    /// 保存图片到相册
    /// 需要授权`Privacy - Photo Library Additions Usage Description`和`Privacy - Photo Library Usage
    /// Description`
    ...
    func bat_save(
        ...
        if oldStatus == .authorized {
            let status = p_excuteSaveImage(intoAlbum: albumName)
    ...
    private func p_createdCollection(albumName: String) -> PHAssetCollection? {
        ...
        guard let _ = try? PHPhotoLibrary.shared().performChangesAndWait({
            createdCollectionID = PHAssetCollectionChangeRequest
        ...
        return PHAssetCollection.fetchAssetCollections(
            withLocalIdentifiers: [createdCollectionID],
            options: nil
        ).firstObject
    }
    ...
    private func p_createdAssets() -> PHFetchResult<PHAsset>? {
        do {
            var assetID = ""
            try PHPhotoLibrary.shared().performChangesAndWait {
                assetID = PHAssetChangeRequest.creationRequestForAsset(from: self)
