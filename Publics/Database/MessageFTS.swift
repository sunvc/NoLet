...
final class MessageFTS: @unchecked Sendable {
    ...
    private let logger = Logger(subsystem: "app.wzs.logger", category: "MessageFTS")
    ...
    func deleteBulk(group: String? = nil, onlyRead: Bool = false, before interval: Double? = nil) async {
        var whereSQL: [String] = []
        var binds: [Any] = []
        if let group {
    ...
    // Triggers are dropped unconditionally before (re)creation so app updates replace
    // any previously-installed (possibly broken) version.
    private static let triggerSQL = """
    DROP TRIGGER IF EXISTS ZFTS_ad;
    DROP TRIGGER IF EXISTS ZFTS_ai;
    DROP TRIGGER IF EXISTS ZFTS_au;
