...
final class PTTStreamingReceiver {
    ...
    private final class ReceiveSession {
        ...
        /// Monotonic wall time of the last AUDIO frame ingested for this
        /// session. Used to detect silent disconnect / dropped END.
    ...
    private func handleAudioFrame(_ frame: PTTFrame) {
        // Server guarantees one active talker per channel. Bind AUDIO to the
        // latest non-ended receive session; queued/paused sessions retain the
        // compressed packet without decoding it.
        guard let session = latestInputSession() else {
            log.debug("dropped AUDIO seq=\(frame.seq) — no input session")
            return
        }
    ...
    private func handleReplayEnd(_ frame: PTTFrame) {
        ...
        // The server replays the ring-buffer starting from seq 0 while live
        // audio continues at the sender's monotonic cursor. After the last
        // replay packet, reset the jitter read cursor so the live tail
        // (which may arrive with seq ≥ replay_end+1) seamlessly follows.
        session.jitter.reset(startingSeq: session.jitter.nextSeq)
    }
    ...
    func activate(sessionID: String) {
        ...
        if engineConfigured, !playerNode.isPlaying {
            do {
                try AVAudioSession.sharedInstance().setActive(true, options: .notifyOthersOnDeactivation)
            } catch {
                log.error("activate: audio session reactivation failed: \(String(describing: error), privacy: .public)")
            }
            playerNode.play()
        }
    ...
    func resume(sessionID: String) {
        ...
        if engineConfigured, !playerNode.isPlaying {
            do {
                ...
                log.error("resume: audio session reactivation failed: \(String(describing: error), privacy: .public)")
            }
            playerNode.play()
        }
        fillScheduledLead(session)
    ...
    private func fillScheduledLead(_ session: ReceiveSession) {
        ...
        // transitions to the background or another participant starts
        // speaking.  Try to recover once — if the audio session is still
        // active the node should restart cleanly.
        if !playerNode.isPlaying {
            do {
                try AVAudioSession.sharedInstance().setActive(true, options: .notifyOthersOnDeactivation)
                playerNode.play()
            } catch {
                log.debug("fillScheduledLead: player node is paused, cannot recover: \(String(describing: error), privacy: .public)")
                return
            }
            // If the engine itself stalled, restart it too.
            if !engine.isRunning {
                do {
                    try engine.start()
                } catch {
                    log.debug("fillScheduledLead: engine restart failed: \(String(describing: error), privacy: .public)")
                    return
                }
            }
        }
