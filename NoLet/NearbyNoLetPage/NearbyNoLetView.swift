...
struct NearbyNoLetView: View {
    ...
    @State private var exportedFileName: String = .init(localized: "文件")
                    ...
                    VStack {
                                    ...
                                    ForEach(viewModel.messages, id: \.id) { message in
                                        MessageBubble(
                                            ...
                                            onSaveFile: { data, name in
                                                ...
                                                if let name = name {
                                                    let ext = URL(fileURLWithPath: name)
                                                        .pathExtension
                                                    exportUTType = UTType(filenameExtension: ext) ??
                                                        .data
                                                } else {
                                                    exportUTType = .data
                                                }
                        ...
                        HStack {
                            ...
                            .onChange(of: selectedPhotoItem) { newItem in
                                ...
                                Task {
                                    if let imageTransfer = try? await item
                                        .loadTransferable(type: ImageData.self)
                                    {
                    ...
                    if let data = message.imageData, let uiImage = UIImage(data: data) {
                        Image(uiImage: uiImage)
                            ...
                            .overlay(
                                GeometryReader { geo in
                                    let width = max(
                                        0,
                                        min(
                                            geo.size.width * CGFloat(message.progress),
                                            geo.size.width
                                        )
                                    )
                            ...
                            .overlay(alignment: .topTrailing) {
                                if message.isTransferring {
                                    Text(verbatim: "\(Int(message.progress * 100))%")
                                        .font(.caption2)
                        ...
                        ZStack(alignment: .leading) {
                            ...
                            GeometryReader { geo in
                                let width = max(
                                    0,
                                    min(geo.size.width * CGFloat(message.progress), geo.size.width)
                                )
                                RoundedRectangle(cornerRadius: 10)
                                    .fill(isFromCurrentUser ? Color.blue : .orange)
                                    .frame(width: width)
                            ...
                            HStack(spacing: 8) {
                                ...
                                VStack(alignment: .leading, spacing: 4) {
                                    ...
                                    if let data = message.fileData {
                                        Text(
                                            "大小: \(ByteCountFormatter.string(fromByteCount: Int64(data.count), countStyle: .file))"
                                        )
                                        .font(.caption)
