    ...
    func body(content: Content) -> some View {
        content
            ...
            .safeAreaInset(edge: .bottom) {
                if let reply = message.value(for: .reply, as: String.self) {
...
struct MessageActionMenu: View {
    ...
    var body: some View {
        Menu {
            Section {
                Button {
                    NCONFIG.copy(message.bodyText.plainText)
                    Toast.copy()
                } label: {
            ...
            if let image = message.value(for: .image), !image.isEmpty {
                Section {
                    Button {
            ...
            if let reply = message.value(for: .reply), !reply.isEmpty {
                Section {
                    Button {
                        showReply = true
                    } label: {
    ...
    static func examples() -> [[AnyHashable: Any]] {
        [
            ...
            [
                ...
                "style": "pay",
                "other": """
                    { "ticket":"NO: 999999999999","icon": "https://favicon.wzs.app/wechat.com" }
                    """,
            ],
        ]
    }
