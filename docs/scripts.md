...
伞兵(NoLet) 支持用 JavaScript 脚本扩展通知行为。脚本在 App 内置的沙盒 JavaScript 运行时中执行，可使用 `fetch`、`crypto`、`storage`、`setTimeout`、`console` 等标准 API，按用途分为四种模式。
...
- **未绑定脚本**：点击仅打开 App。
- **「打开主 App」开关**：控制点击是否跳转到主 App（默认开）。关闭后，绑定了脚本的按钮只在通知界面内执行脚本、不跳转；未绑定脚本时点击无反应。
...
「脚本列表」页右上角云朵图标进入插件中心：所有用户共享的 JS 脚本市场，数据存于 App 的 iCloud 公共库。

#### 浏览与安装

- 支持按名称、简介、标签关键词搜索，按脚本类型（语音/处理器/动作/插件）筛选，列表按发布时间倒序、滚动翻页。
- 点进详情可以看到作者、类型、大小、SHA256 和**完整源码**。脚本来自其他用户，运行时具备网络请求、读写 `storage` 等能力，**安装前请务必查看源码确认安全**。
- 点「安装到我的脚本」会再次做语法与入口校验，然后保存到本地脚本列表：
  - 本地已有相同内容（SHA256 一致）时提示"已安装"，不会重复保存；
  - 文件名冲突时自动改名（`xxx.js` → `xxx-2.js`）。

#### 上传自己的脚本

- 工具栏「上传脚本」：从本地脚本中选择一个，填写名称、简介、标签后上传。上传前会用示例参数真实校验一次，校验不通过的脚本不能上传。
- 需要设备登录 iCloud；每个用户最多上传 50 个脚本。
- 「我的上传」页面管理自己发布的脚本，可以删除云端记录；删除后其他人不可见，已安装的用户不受影响。

> 插件中心内容完全开放、没有人工审核，只分享你信任的脚本，只安装你看过源码的脚本。
