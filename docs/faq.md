...
排查都正常后还有问题可在[伞电(Nolet) 问题反馈群](https://t.me/PushToMe)反馈。

#### DeviceToken显示未知

*平台差异：apple（设备与苹果 APNs 的连接问题）。鸿蒙端对应检查推送 Token 是否正常。*
...
在 iOS 14.5 及更高版本中，由于权限更严格，App 无法在收到推送时自动将内容复制到剪贴板。<br/>
作为替代方案，下拉推送或在锁屏界面左滑推送点查看即可自动复制，或点击弹出的推送复制按钮。

#### 再次打开 App 默认进入历史消息列表

重新打开 App 时会回到上次打开的页面。<br>
在退出 App 时停留在消息历史页面，下次打开 App 就会默认进入消息历史列表。
...
#### 鸿蒙端为什么副标题、定时、卡片样式、定位等不生效？

平台差异：这些能力（`subtitle`、`style` 五种卡片、`other` 扩展、`reply`、`location`、`level`、脚本与插件等）目前仅 Apple 端支持（`call` 为全平台：鸿蒙用数字 10–60 控制铃声时长）。鸿蒙端统一使用通用卡片并按 Markdown 渲染正文；两平台通用的字段见各文档中的 `全平台` 标签。
